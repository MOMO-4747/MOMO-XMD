...
const commandFooter = () => `\n\n> ❑ Powered by MOMO47 ❑`;
const boxStyles = {
    arched: { top: "╭◆", bottom: "╰◆", bullet: "◆", foot: "◇" },
    downloader: { top: "╭━━❐━⪼", bottom: "╰━━❑━⪼", bullet: "┇", foot: "❑" },
    diamond: { top: "╭◇", bottom: "╰◇", bullet: "◇", foot: "✦" },
    star: { top: "╭★", bottom: "╰★", bullet: "✦", foot: "◉" },
    square: { top: "┏▣", bottom: "┗▣", bullet: "┃", foot: "◆" },
    circle: { top: "╭○", bottom: "╰○", bullet: "○", foot: "★" },
    spear: { top: "╭➤", bottom: "╰➤", bullet: "➤", foot: "◇" },
    crown: { top: "╭♛", bottom: "╰♛", bullet: "♛", foot: "❖" },
    bolt: { top: "╭⚡", bottom: "╰⚡", bullet: "⚡", foot: "✧" },
    flower: { top: "╭✿", bottom: "╰✿", bullet: "✿", foot: "◈" },
    wave: { top: "╭≈", bottom: "╰≈", bullet: "≈", foot: "▣" },
    cross: { top: "╭✚", bottom: "╰✚", bullet: "✚", foot: "❖" },
    arrow: { top: "╭➜", bottom: "╰➜", bullet: "➜", foot: "✦" },
    diamond2: { top: "╭⬥", bottom: "╰⬥", bullet: "⬥", foot: "○" },
    triangle: { top: "╭▲", bottom: "╰▼", bullet: "▲", foot: "◆" },
    music: { top: "╭♫", bottom: "╰♫", bullet: "♫", foot: "◇" },
    flame: { top: "╭🔥", bottom: "╰🔥", bullet: "🔥", foot: "✧" },
    shield: { top: "╭⬢", bottom: "╰⬢", bullet: "⬢", foot: "◉" },
    bars: { top: "╭╾", bottom: "╰╼", bullet: "╾", foot: "★" },
    key: { top: "╭⚿", bottom: "╰⚿", bullet: "⚿", foot: "❑" },
    leaf: { top: "╭❧", bottom: "╰❧", bullet: "❧", foot: "✦" }
};
const formatBox = (content, type = "arched", symbol, footerSymbol) => {
    const style = boxStyles[type] || boxStyles.arched;
    const bullet = symbol || style.bullet;
    const lines = String(content).split("\n");
    const foot = footerSymbol || style.foot;
    return `${style.top}\n${lines.map(line => `${bullet} ${line}`).join("\n")}\n${style.bottom}\n\n> ${foot} Powered by MOMO47 ${foot}`;
};
const commandStyles = ["square", "star", "diamond", "downloader", "circle", "spear", "crown", "bolt", "flower", "wave", "cross", "arrow", "diamond2", "triangle", "music", "flame", "shield", "bars", "key", "leaf"];
const commandOrder = ["menu", "ping", "runtime", "restart", "pair", "getpp", "listgroups", "listchat", "setgroupdesc", "antileft", "alive", "anticall", "antilink", "antiviewonce", "autoreact", "autolikestatus", "autoviewstatus", "chatbot", "alwaysonline", "autosavestatus", "autoviewonce", "autorecording", "autotyping", "block", "unblock", "desc", "repo", "channel", "mode"];
const commandStyle = (command, phase = "success") => {
    const base = Math.max(0, commandOrder.indexOf(command));
    if (phase === "usage") return commandStyles[(base + 7) % commandStyles.length];
    if (phase === "error") return commandStyles[(base + 13) % commandStyles.length];
    return commandStyles[base % commandStyles.length];
};
const commandBox = (command, phase, content, symbol, footerSymbol) => formatBox(content, commandStyle(command, phase), symbol, footerSymbol);
const plainExample = (command, args) => `Example\n.${command} ${args}`;
const toggleUsage = (command) => `Example\n.${command} on\n.${command} off`;
const commandUsageBox = (command, args = ["on", "off"]) => {
    const styleName = commandStyle(command, "usage");
    const style = boxStyles[styleName];
    const options = args.map(arg => `${style.bullet} .${command} ${arg}`).join("\n");
    return `Example\n.${command} ${args[0]}\n.${command} ${args[1]}\n\n${formatBox(options, styleName, undefined, style.foot)}`;
};
...
async function startBot() {
    ...
    sock.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            const platformStr = (() => {
                const explicit = String(process.env.DEPLOY_PLATFORM || process.env.PLATFORM || '').trim().toLowerCase();
                if (explicit.includes('katabamp')) return 'Katabamp';
                if (explicit.includes('panel') || process.env.PANEL_HOST || process.env.PANEL_APP) return 'Panel';
                if (explicit.includes('heroku') || process.env.HEROKU_APP_NAME) return 'Heroku';
                if (explicit.includes('linux') || process.env.VPS_HOST || config.host === 'VPS') return 'Linux';
                return 'Linux';
            })();
            const msg = `┏━━━━━━✧ CONNECTED ✧━━━━━━━
┃✧ Bot: MOMO-XMD
...
╰━━━━━━━━━━━━━━━━╯`;
            ...
            // Follow the four configured channels when this Baileys build exposes
            // the newsletter API. Never claim success when WhatsApp/API support is absent.
            try {
                if (typeof sock.newsletterMetadata === "function" && typeof sock.newsletterFollow === "function") {
                    for (const inviteUrl of (config.channels || []).slice(0, 4)) {
                        const inviteCode = inviteUrl.match(/channel\/([^/?]+)/)?.[1];
                        if (!inviteCode) continue;
                        const metadata = await sock.newsletterMetadata("invite", inviteCode);
                        const jid = metadata?.id || metadata?.jid;
                        if (jid) await sock.newsletterFollow(jid);
                    }
                    console.log("[CHANNELS] Follow attempts completed for configured channels");
                } else {
                    console.log("[CHANNELS] Newsletter follow API is unavailable in this Baileys runtime");
                }
            } catch (e) {
                console.log("[CHANNELS] Follow attempt failed:", e.message);
            }
        }
    });
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        switch (command) {
            ...
            case "antileft": {
                ...
                if (!["on", "off"].includes(opt)) { await sock.sendMessage(from, { text: commandUsageBox(command) }, { quoted: msg }); break; }
            ...
            case "mode": {
                ...
                } else await sock.sendMessage(from, { text: commandUsageBox(command, ["public", "self"]) }, { quoted: msg });
                break;
            }
            ...
            case "antiviewonce": {
                ...
                if (["on", "off"].includes(opt)) {
                    ...
                    await sock.sendMessage(from, { text: commandBox(command, "success", `${command.toUpperCase()} ${opt === "on" ? "ENABLED 🟢" : "DISABLED 🔴"}`, opt === "on" ? "✅" : "❌", opt === "on" ? "★" : "◇") }, { quoted: msg });
                } else await sock.sendMessage(from, { text: commandUsageBox(command) }, { quoted: msg });
                break;
            }
